...
| `utils/openCodeTurnActivity.ts` | Pure decision for the OpenCode wedged-turn detector: `checkIsWedgedTurn` — a delivered prompt (`awaitingResponse`) that idled with no assistant activity (`sawActivity`, set ONLY by an assistant `message.updated` — never the echoed user-prompt parts), NOT during a compaction and NOT with a pending provider retry, means OpenCode accepted the prompt but never ran a turn → the bot auto-recovers |
| `utils/wedgeRecovery.ts` | Pure loop-guard for recovering a wedged OpenCode session: `decideWedgeRecovery` → `restart` (a cached prompt to replay AND no attempt yet) else `giveUp`. The bot's `handleNoResponse` does the restart (fresh session + replay) at most once per prompt episode, else surfaces `agent.no_response` |
