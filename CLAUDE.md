...
- **Agent scheduling tools (injected).** Separately from that user hierarchy,
  ...
  (agent→user per-message text delivery: each array item posted as its OWN Telegram
  message, never merged — the opt-in path behind a per-item news digest; same
  dir/thread scope) and is bot-owned
...
| `utils/messageSendService.ts` | Reusable impure sender behind `send_messages_to_user` (`createSendMessagesToThread<TTarget>`): resolves the target, then delivers each input string as its OWN message (splitting an over-cap one via `splitMessage`, skipping a blank), preserving order. `bot.ts` composes it over the paced `replyChunkWithFallback` + `renderAgentHtml`; a total send failure returns an error (not a false "delivered 0"), a partial one stays `ok` but names the landed/attempted split. `maxDiscreteMessages` (50) is the per-call cap (enforced at the MCP schema) |
