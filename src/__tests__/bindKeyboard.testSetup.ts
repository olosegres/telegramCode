/**
 * Side-effect-only setup for `bindKeyboard.test.ts`.
 *
 ...
 * hoisted imports), so `parseEnv()` succeeds and the pure `buildBindKeyboard`
...
const tempWorkRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'bind-keyboard-'));
