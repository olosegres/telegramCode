...
| `utils/openCodeTurnActivity.ts` | Pure decision for the OpenCode wedged-turn detector: `checkIsWedgedTurn` — a delivered prompt (`awaitingResponse`) that idled with no assistant activity (`sawActivity`), NOT during a compaction and NOT with a pending provider retry, means OpenCode accepted the prompt but never ran a turn → surface `agent.no_response` instead of hanging silently |
...
| `openCodeAdapter.ts` | OpenCode via HTTP + SSE (POST prompts; ONE multiplexed `/global/event` stream for the whole server, every event parsed once + routed by envelope `directory` + `sessionID`). **Wedged-turn detection** (live 2026-08-15, the my-news digest schedule): a bloated session accepted every prompt (`prompt_async` 204) but its agent loop exited at once — `session.idle` arrived with ZERO assistant activity, so the topic looked silently hung («агент по триггеру не начинает работу»). A prompt arms `awaitingTurnResponse`; the first own-session idle with no `sawTurnActivity` (any assistant `message.updated`/part, own or child) emits `agent.no_response` («session may be stuck — /new»). Guarded against a legit compaction idle and a still-pending provider retry (both idle with no text). Pure decision in `utils/openCodeTurnActivity.ts` (`checkIsWedgedTurn`) |
