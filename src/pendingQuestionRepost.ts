 ...
 * was the last thing sent — otherwise re-posting would either be pointless (no
 * pending question) or loop (re-posting in reaction to its own send).
 */
export function checkShouldRepostPendingQuestion(input: RepostDecisionInput): boolean {
  return input.isQuestionPending && !input.wasLastSendTheQuestion;
}
